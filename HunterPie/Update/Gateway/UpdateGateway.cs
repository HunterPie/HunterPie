    ...
    PoogieLocalizationConnector localizationConnector)
{
    ...
    public async Task<bool> DownloadVersionAsync(string version, string output, DownloadEventHandler callback)
    {
        Response.Success resp = await _versionConnector.Download(version);

        if (resp.StatusCode != HttpStatusCode.OK)
            throw new NetworkException($"Failed to request package file, was expecting status code 200 but got {resp.StatusCode}");

        await resp.DownloadAsync(output, callback);
