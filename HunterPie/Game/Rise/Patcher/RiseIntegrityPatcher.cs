...
namespace HunterPie.Game.Rise.Patcher;

internal class RiseIntegrityPatcher(
    IContext context
)
{
    ...
    public async Task Patch()
    {
        ...
        for (int i = 0; i < crcFuncs.Length; i++)
        {
            ...
            _logger.Debug($"Patched 0x{crcFunc:X}");
        }
    }
    ...
    /// </summary>
    public async Task PatchProtectVirtualMemoryAsync()
    {
        ...
        {
            _logger.Error("Failed to find ntdll address");
            return;
        }

        _logger.Debug($"Found ntdll address at {ntdllAddress:X}");
        ...
        {
            _logger.Error("Failed to find ntdll::NtProtectVirtualMemory address");
            return;
        }

        _logger.Debug($"Found ntdll::NtProtectVirtualMemory address at {ntProtectVirtualMemory:X}");
