...
internal static class WorldIntegrityPatcher
{
    ...
    public static void Patch(IContext context)
    {
        ...
        void PatchCrc(string addressName1, string addressName2, string originalFuncName1, string originalFuncName2)
        {
            ...
            const int jmpInstructionLength = 1 + 4;
            byte[] patch = new byte[jmpInstructionLength + 1];

            // JMP rel32: Jump near, relative, RIP = RIP + 32-bit displacement sign extended to 64-bits
            patch[0] = 0xE9;
            int rel32 = (int)(crc2 - crc1 - jmpInstructionLength);

            BitConverter.TryWriteBytes(patch.AsSpan(1, 4), rel32);

            // NOP
            patch[jmpInstructionLength + 0] = 0x90;
