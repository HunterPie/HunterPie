...
using HunterPie.Features.Extensions.Entity;
...
namespace HunterPie.Features.Extensions.Loader;
    ...
    LocalThemeRepository localThemeRepository)
{
    ...
    {
        IReadOnlyCollection<LocalThemeManifest> themes = await localThemeRepository.ListAllAsync();

        IOrderedEnumerable<LocalThemeManifest> enabledThemes = themes
            .Where(it => config.Client.Themes.Contains(it.Manifest.Id))
            .OrderByDescending(it => config.Client.Themes.IndexOf(it.Manifest.Id));
    ...
    public void UnloadAllThemes()
    {
        ...
        {
            LocalThemeManifest? manifest = localThemeRepository.FindBy(themeId);
