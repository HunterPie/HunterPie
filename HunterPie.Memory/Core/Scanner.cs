...
public class Scanner
{
    ...
    public static Scanner Create(string name) => new() { Name = name };
    ...
    public Scanner GetMemory()
    {
        ...
        _ = Kernel32.ReadProcessMemory(pHandle, process.MainModule.BaseAddress, memory, memory.Length, out _);
        return this;
    }
    ...
    public Scanner FindSignatures()
    {
        ...
        for (int i = 0; i < memory.Length && Signatures.PatternsLeft > 0; i++)
        {
            if (pageSize == 0)
            {
                ...
                _ = Kernel32.VirtualQueryEx(pHandle, process.MainModule.BaseAddress + i, out MEMORY_BASIC_INFORMATION pageInformation, (uint)Marshal.SizeOf<MEMORY_BASIC_INFORMATION>());
            ...
            var span = new Span<byte>(memory, i, Signatures.MaximumLength);
